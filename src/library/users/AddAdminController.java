...
//store date
    ...
    private void addAdmin() {
        ...
        if (userFirstName.isEmpty() || userLastName.isEmpty() || userAddress.isEmpty() || userMobile.isEmpty()||userEmail.isEmpty() || userPassword.isEmpty()) {
            ...
            String registerAdmin = "INSERT INTO user_account (user_type,first_name,last_name,address,mobile,email,password,is_blocked) VALUES(" + "'admin'," +
                    "'" + userFirstName + "'," +
                    "'" + userLastName + "'," +
                    "'" + userAddress + "'," +
                    "'" + userMobile + "'," +
                    ...
                    "" + true + "" + ")";
            try {
                stmt = connectDB.prepareStatement(registerAdmin);
                stmt.executeUpdate(registerAdmin);
                Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                alert.setHeaderText(null);
                alert.setContentText("New Admin Registered Successfully");
                alert.showAndWait();
                cancel();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    @FXML
    private void cancel (){
        Stage stage = (Stage) addAdminPane.getScene().getWindow();
        stage.close();
    }
}
